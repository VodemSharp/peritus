...
`PasswordChangeFeature`, `SessionRevokeFeature`, `TokenRefreshFeature`, and the shared
`UserService.UpdateAsync`. To find every current caller, search `db.*.Update(`:

```bash
grep -rn "\.Update(" src/modules/Peritus.Identity/
```
